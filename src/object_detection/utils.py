...
def draw_object_mask(
    ...
    if len(mask.shape) == 2:
        ...
        color_mask[mask == 1] = (
            target_highlight_color  # Set the color for the segmented area
        )
...
def draw_object_box(
    ...
    bottom: int,
    box_text: str,
    target_highlight_color: Tuple[int, int, int],
) -> np.ndarray:
    """
...
def get_frame_box_dimensions_delta(
    left: int, top: int, right: int, bottom, frame_width: int, frame_height: int
) -> Tuple:
    """
